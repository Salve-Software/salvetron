...
import type { MakoEvent } from '@salve-software/mako-types'
...
export function startWsServer(port: number) {
  ...
  wss.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      console.error(`Port ${port} already in use. Set MAKO_PORT env var to change port.`)
      process.exit(1)
    }
  })
  ...
  wss.on('connection', (socket) => {
    socket.on('message', (raw) => {
      try {
        const event: MakoEvent = JSON.parse(raw.toString())
        dispatch(event)
      } catch {}
    })
...
function dispatch(event: MakoEvent) {
  switch (event.type) {
    case 'device_info':
    case 'project_info':
